...
class StatusStation(Resource):
	...
	def get(self, user=None, sub=None, role=None):
		...
		try:
			patients = {}
			e = DBEvent.query.join(DBBodyLocation, DBBodyLocation.id == DBEvent.body_location_id)\
				.join(DBDevice, DBDevice.id == DBEvent.device_id)\
				.join(DBPatient, DBPatient.id == DBDevice.patient_id)\
				.filter(
					DBEvent.instance == None,
					DBEvent.range_end == None,
					# DBEvent.latest != None,
					DBDevice.last_seen > (datetime.utcnow() - timedelta(seconds=ALERT_INTERVAL_SECONDS)),
					DBDevice.patient_id != None,
					DBBodyLocation.sensor_serial != SENSOR_UNASSIGNED,
					DBPatient.deleted == False,
					DBEvent.message != SENSOR_REMOVED,
					DBPatient.medical_record_number == DBEvent.medical_record_number
			).add_columns(
					DBPatient.unit_floor,
					DBPatient.name,
					DBPatient.last_name,
					DBPatient.medical_record_number,
					DBBodyLocation.battery,
					DBPatient.room,
			).all()
			if e:
				for v in e:
					...
					patients[v.medical_record_number]['name'] = str(v.name) + ' ' + (v.last_name or '')
			...
			for p, last_seen in ps:
				...
				patients[p.medical_record_number]['name'] = p.name + ' ' + (p.last_name or '')
