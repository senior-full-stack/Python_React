...
class RepositionReport(Resource):
	@swagger.operation(
		notes='none',
		nickname='get reposition reports for patient OR unit',
		parameters=[
			{
				"name": "start",
		...
		responseMessages=[
			{
				"code": 200,
				"message": "An array of reposition reports for the given time period"
			},
			{
				"code": 400,
				"message": "Bad request"
	...
	def get(self, user=None, sub=None, medical_record_number=None, role=None):
		...
		try:
			...
			if medical_record_number:
				reposition_query = DBAlarmResponse.query.filter_by(pa_id=medical_record_number).order_by(DBAlarmResponse.date, DBAlarmResponse.hour)
				...
				if start:
					reposition_query = reposition_query.filter(DBAlarmResponse.date >= start.date())
				if end:
					reposition_query = reposition_query.filter(DBAlarmResponse.date <= end.date())


			elif unit_floor:
				reposition_query = DBAlarmResponse.query.filter_by(unit_floor=unit_floor).order_by(DBAlarmResponse.date, DBAlarmResponse.hour)
				...
				reposition_query = DBAlarmResponse.query.order_by(DBAlarmResponse.date, DBAlarmResponse.hour)
			...
			repositions = reposition_query.all()
			...
			for i in range(0, 24):
				patient = [x.patient_repositions for x in repositions if x.hour == i]
				if patient:
					total_patient[i] = sum(patient)

				caregiver = [x.caregiver_repositions for x in repositions if x.hour == i]
				if caregiver:
					total_caregiver[i] = sum(caregiver)

				total = [x.total_repositions for x in repositions if x.hour == i]
				...
				if medical_record_number:
					...
					query_string = 'select COALESCE(max(active),0) from (select sum(number_active) as active, unit_floor, date from patients_active as ps where hour = ' \
						+ str(i) + " and date >= '" + str(start.date()) + "' and date <= '" + str(end.date()) + "' group by date, unit_floor) as NAUD;"
					unique_active = conn.execute(query_string)
					unique_active = unique_active.fetchone()
					unique_active_patients[i] = int(unique_active.items()[0][1])
			...
			query_string = 'select GROUP_CONCAT(data) as json from reposition_report_data where reposition_report_data.all = 1 and date >= "2016-11-05"'

			query_string = 'select GROUP_CONCAT(data) as json from reposition_report_data where '
			...
			if medical_record_number:
				...
				query_string += 'reposition_report_data.all = "1" '
			...
			last_report_data = DBRepositionReportData.query.order_by(DBRepositionReportData.date.desc()).first()
			...
			response = {'reposition_reports' : response, 'total_amount': len(response)}
		...
		if status_code == status.HTTP_200_OK and format and format.lower() == CSV:
			return self.output_csv(response['reposition_reports'], status_code, CSV_HEADERS)
		else:
			response['reposition_reports'] = response['reposition_reports'][:1000]
